...
export const GET: APIRoute = async () => {
  ...
  if (!data && (!error || error.code === 'PGRST205' || error.code === '42P01')) {
    return respond(200, { version: 1, state: 'unconfigured', activity: null, music: null });
  }
  if (error) return respond(503, { error: 'Presence storage is unavailable' });

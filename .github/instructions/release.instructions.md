...
- Tag builds with no Apple credentials may publish an ad-hoc signed, unnotarized release whose notes clearly explain the Gatekeeper limitation. A partial credential set must fail instead of silently falling back.
- Only describe a release as Developer ID signed or notarized after Developer ID, notarization, stapling, and Gatekeeper checks pass.

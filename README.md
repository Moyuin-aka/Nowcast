...
- **应用内更新**：从菜单栏或设置窗口点击「检查更新…」，验证签名后安装并重启。
...
后续版本可从菜单栏或设置窗口点击「检查更新…」直接安装。Nowcast 默认不会在后台检查更新。
...
使用 macOS 与完整 Xcode 工具链。项目基于 Swift / AppKit / SwiftUI；应用内更新使用 Sparkle 2，其他运行时功能不依赖第三方库。
